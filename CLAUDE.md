...
Chapter 1 ("Machine Learning Foundations") covers: why ML exists (vs. traditional rule-based programming), the three types of ML (Supervised, Unsupervised, Reinforcement) and their sub-types (Classification/Regression, Clustering/Dimensionality Reduction), a structured checklist for approaching a vague business problem and framing it as an ML problem, a conceptual overview of each of the three learning types, and Parametric vs. Non-Parametric models (a second, independent axis for categorizing algorithms — fixed-size distilled formula vs. grows-with-data). Written to `01-Machine Learning Foundations/theory.md`, with hands-on scenario-based practice problems in `types_of_ml_practice.ipynb` — Varun asked for these filled in with working code + explanations (not scaffold-only) since this is early learning phase, each cell showing the *shape* of that ML type's data (labeled data for supervised, unlabeled for unsupervised, agent/action/reward for RL) with meaning + expected-output comments per line, same convention as `python-self-learning`'s reference notebooks.
